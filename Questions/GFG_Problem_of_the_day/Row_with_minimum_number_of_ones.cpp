...
to complete the function minRow() which takes the two integers n, 
m as well as the 2D binary matrix a as input parameters and returns 
the minimum index of the row which contains the least number of 1's.
...
int main() {
    ...
    while (t--) {
        ...
        cout<<"Enter the values of 0's and 1's: ";
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
                cin >> A[i][j];
            }
        ...
        Solution ob;
        cout<<"The minimum number of 1's row are: ";
